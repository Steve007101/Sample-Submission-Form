...
<!--
  ...
  sample update page. Intended to be called by another php file.
  Only creates a $record_table variable then ends. Outputs 2 tables
  for formatting purposes on the web page.
  -->
<?php
...
$record_table .= "<input type='hidden' id='selected_id' name='selected_id' value='"."000".ltrim($row[0], "0")."'>";
...
if ( $row['sorigin'] == null || $row['sorigin'] == ""
  || $row['sorigin'] == "Supplier" || $row['sorigin'] == "Customer") {
    $record_table .= ($row['ocompany'] != "" && $row['ocompany'] != $row['sorigin'] ?
                    "<tr><th>Origin</th><td>".$row['ocompany']."</td></tr>" : "");
    if ($row['oaddress'] != "") {
...
if ($row['srecipient'] == null || $row['srecipient'] == ""
    ...
    $record_table .= "<tr><th>Address</th><td>".($row['rco'] != "" ? $row['rco']."<br>" : "")
      .$row['raddress']."<br>"
      ...
      .$row['rcountry']) : "")
      ."</td></tr>";
    $record_table .= ($rphone_output != "" && $rphone_output != "() -" ? "<tr><th>Phone</th><td>".$rphone_output."</td></tr>" : "");
    $record_table .= ($row['remail'] != "" ? "<tr><th>Email</th><td>".$row['remail']."</td></tr>" : "");
    ...
    $record_table .= ($row['rshipping'] != "" ? "<tr><th>Shipping</th><td>".$row['rshipping']."</td></tr>" : "");
}
...
for ($x = 1; $x <= $max_possible_photos; $x++) {
  if ($row['photo'.$x] != null && $row['photo'.$x] != "") {
      $record_table .= "<tr><th>Photo ".$x."</th><td><a href='".$absolute_dir."/".$photos_dir."/"
      .$row['photo'.$x]."' target='_blank'>".$row['photo'.$x]."</a></td></tr>";
  }
}
// lab cover letter link here
// hide initially, the update page will make this visible if distributor loads page
if ($row['srecipient'] == "Lab" || $row['srecipient'] =="St Louis Testing (Lab)" || $row['srecipient'] == "UMSL Labs") {
  $record_table .= "<tr id='lab_cover' hidden><th>Lab Cover Letter</th><td><a href='".$absolute_dir."/".$lab_cover_letter_dir."/"
  .$row['lab_cover']."' target='_blank'>".$row['lab_cover']."</a></td></tr>";
}
$record_table .= "</table>";


// hidden values to pass for tracking, lab report, and lab cover letter
